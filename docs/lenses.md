...
> Deep-dives for each lens in `trace·lab`. The [README](../README.md) is the
> short version; this is the long one. Mechanics here drift faster than the
> top-level story, so it lives out of the front door on purpose.
